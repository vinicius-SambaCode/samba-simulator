# -*- coding: utf-8 -*-
"""
Rotas do módulo de Simulados (Sprint 1)
- Criar simulado, alocar turmas, definir cotas, atribuir professores
- Alimentar questões: unitário e colado (txt)
- Validar quantidade de alternativas conforme options_count (4|5)
- Travas de status: collecting / review / locked
"""
...
# Estruturas escolares (SchoolClass)
...
def _teacher_can_feed(db: Session, exam: Exam, current_user: User, class_id: int, discipline_id: int):
    """
    Verifica se current_user (professor) está atribuído ao recorte exam + turma + disciplina.
    Evita que professor alimente turmas/disc. de outro colega.
    """
...
def _next_order_idx(db: Session, exam_id: int) -> int:
    """
    Retorna o próximo order_idx disponível para o simulado.
    Evita violar UNIQUE(exam_id, order_idx).
    """
...
def _ensure_selection_link(
    ...
    """
    Garante um registro em ExamQuestionLink para esta questão.
    - Se já existir, não duplica.
    - Se houver correct_label (e answer_source == TEACHERS), cria com próximo order_idx.
    - Se não houver correct_label:
        * por padrão, não cria (mantemos regra de exigir correta no fluxo TEACHERS).
        * se force_create_without_correct=True, cria mesmo sem correta.
    """
    existing = db.query(ExamQuestionLink).filter_by(
        exam_id=exam.id, question_id=question_id
    ).first()
    if existing:
        # atualiza correct_label se veio uma nova
        if correct_label and existing.correct_label != correct_label:
            existing.correct_label = correct_label
            db.add(existing)
        return

    # Criar somente se houver correta, ou se for forçado
...
def assign_teachers(exam_id: int, payload: AssignTeachersIn, db: Session = Depends(get_db)):
    ...
    # valida se o par (class_id, discipline_id, teacher_user_id) é coerente com o mapeamento institucional
    for it in payload.items:
...
def create_question(exam_id: int, payload: QuestionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    exam = _get_exam_or_404(db, exam_id)
    _assert_status(exam, [ExamStatus.COLLECTING])
    _teacher_can_feed(db, exam, current_user, payload.class_id, payload.discipline_id)

    # Validar alternativas conforme options_count
    ...
    # Se a fonte do gabarito for TEACHERS, aceitar a correta aqui (opcional)
    if exam.answer_source == AnswerSource.TEACHERS:
        if payload.correct_label:
            ...
            # Cria link com order_idx sequencial (evita UNIQUE collision)
            _ensure_selection_link(db, exam, q.id, corr)

    db.commit()
    return {"detail": "Questão registrado."}


@router.post("/{exam_id}/questions/paste", dependencies=[Depends(require_role("TEACHER"))])
def paste_questions(exam_id: int, payload: BulkPasteIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    exam = _get_exam_or_404(db, exam_id)
    _assert_status(exam, [ExamStatus.COLLECTING])
    _teacher_can_feed(db, exam, current_user, payload.class_id, payload.discipline_id)
    ...
    for item in parsed:
        ...
        if exam.answer_source == AnswerSource.TEACHERS:
            ...
            if corr:
                ...
                labels = {o["label"] for o in opts}
                if corr in labels:
                    # Cria link com order_idx sequencial
                    _ensure_selection_link(db, exam, q.id, corr)
...
def exam_progress(exam_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Monta visão por disciplina (quota) e por turma, e total submetido
...
def lock_exam(exam_id: int, db: Session = Depends(get_db)):
    ...
    # 1) Verifica se cotas foram atendidas
    quotas = {q.discipline_id: q.quota for q in exam.discipline_quotas}
    counts = {d: 0 for d in quotas.keys()}
    ...
    # 2) Verifica se TODAS as questões têm exatamente options_count alternativas
    for q in exam.questions:
    ...
    # 3) Se a fonte do gabarito for TEACHERS, exigir que cada questão selecionada tenha correct_label
    if exam.answer_source == AnswerSource.TEACHERS:
        # se não houver seleção explícita, crie seleção completa com ordem natural
        if not exam.selection:
            order = 1
            for q in sorted(exam.questions, key=lambda x: x.id):
                # em um fluxo ideal, já haverá links criados na coleta;
                # se não houver, exigimos correct_label nesta etapa
                raise HTTPException(status_code=400, detail=f"Questão {q.id} sem gabarito (correct_label).")
                order += 1

    # Transiciona para LOCKED
