<template>
  <div class="page">
    ...
    <!-- Loading / erro / bloqueado -->
    <div v-if="loadingExam" class="loading-state fade-in" :class="{ ready: mounted }" style="--d:.06s">
      <Icon name="lucide:loader-2" class="w-5 h-5 animate-spin text-gray-400" /><span>Carregando simulado...</span>
    </div>
    <div v-else-if="!exam" class="empty-state fade-in" :class="{ ready: mounted }" style="--d:.06s">
      <Icon name="lucide:file-x" class="w-10 h-10 text-gray-200" /><p>Simulado não encontrado ou sem acesso.</p>
    </div>
    <div v-else-if="exam?.status !== 'collecting'" class="locked-state fade-in" :class="{ ready: mounted }" style="--d:.06s">
      ...
      <p class="locked-title">{{ exam.status==='locked'?'Simulado travado':exam.status==='generated'?'Cadernos gerados':'Simulado publicado' }}</p>
    ...
    <template v-else>
      ...
      <Transition name="pop">
        <div v-if="enviado" class="sent-banner">
          ...
          <div class="card" :class="editandoId!==null?'card--editing':''">
            <div class="card-header" :class="editandoId!==null?'card-header--editing':''">
              <div class="card-title">
                <Icon :name="editandoId!==null?'lucide:pencil':'lucide:plus-circle'" class="w-4 h-4"
                  :class="editandoId!==null?'text-blue-600':'text-gray-500'" />
            ...
            <div class="p-4 flex flex-col gap-4">
              ...
              <div class="field">
                <div class="field-label-row">
                  ...
                  <div class="toolbar">
                    <button class="toolbar-btn" @click="showLatexHelp=!showLatexHelp" title="Inserir fórmula LaTeX">
                      <span class="math-sym">∑</span> Fórmula
                    </button>
                    <label class="toolbar-btn" title="Imagem no enunciado">
                      <input type="file" accept="image/*" class="hidden"
                        @change="uploadImagem($event,'stem')" />
                      <Icon name="lucide:image" class="w-3.5 h-3.5"
                        :class="uploadingImagem==='stem'?'animate-pulse text-blue-500':''" />
                      {{ uploadingImagem==='stem'?'Enviando…':'Imagem' }}
                    </label>
                  </div>
                </div>
                <textarea ref="stemRef" v-model="novaQuestao.stem" rows="4"
                  placeholder="Digite o enunciado. Use $formula$ para LaTeX inline ou $$formula$$ para bloco."
                ...
                <Transition name="slide-down">
                  <div v-if="stemPreview" class="preview-box">
                    ...
                    <div class="question-content text-sm text-gray-800" v-html="stemPreview" />
                  </div>
                </Transition>
                <!-- Imagens já salvas no servidor (editando) -->
                <div v-if="imagensDoEnunciado.length" class="images-strip">
                  <div v-for="img in imagensDoEnunciado" :key="img.id" class="image-thumb">
                    <img :src="img.url" :alt="`img ${img.id}`" />
                    <button v-if="editandoId!==null" class="img-del" @click="deletarImagem(img.id)">
                      <Icon name="lucide:x" class="w-3 h-3" />
                    </button>
                  </div>
                </div>
                <!-- Imagens pendentes (antes de salvar) -->
                <div v-if="imagensPendentes.filter(i=>i.context==='stem').length" class="images-strip">
                  <div v-for="(img, idx) in imagensPendentes.filter(i=>i.context==='stem')" :key="idx" class="image-thumb image-thumb--pending">
                    <img :src="img.previewUrl" alt="preview" />
                    <button class="img-del" @click="imagensPendentes.splice(imagensPendentes.indexOf(img),1);URL.revokeObjectURL(img.previewUrl)">
                      <Icon name="lucide:x" class="w-3 h-3" />
                    </button>
              ...
              <!-- Ajuda LaTeX -->
              <Transition name="slide-down">
                <div v-if="showLatexHelp" class="latex-help">
                  <div class="latex-help-header">
                    <span>Clique em um exemplo para inserir</span>
                    <button @click="showLatexHelp=false"><Icon name="lucide:x" class="w-3.5 h-3.5" /></button>
                  </div>
                  <div class="latex-grid">
                    <button v-for="ex in latexExamples" :key="ex.code" class="latex-ex" @click="inserirLatex(ex.code)">
                      <code>{{ ex.code }}</code>
                      <span>{{ ex.label }}</span>
                    </button>
                  </div>
              ...
              <div class="field">
                ...
                <div class="opts-list">
                  <div v-for="(_, i) in novaQuestao.options.slice(0, exam.options_count)" :key="i" class="opt-row">
                    <button class="opt-sel"
                      :class="novaQuestao.correct_label===letras[i]?'opt-sel--on':''"
                    ...
                    <input v-model="novaQuestao.options[i].text" type="text"
                      ...
                      class="field-input"
                      :class="novaQuestao.correct_label===letras[i]?'field-input--correct':''" />
                    <!-- Imagem por alternativa (só quando editando) -->
                    <label class="opt-img-btn" :title="`Imagem para ${letras[i]}`">
                      <input type="file" accept="image/*" class="hidden"
                        @change="uploadImagem($event,`option_${letras[i]}`)" />
                      <Icon name="lucide:image" class="w-3 h-3"
                        :class="uploadingImagem===`option_${letras[i]}`?'animate-pulse text-blue-500':'text-gray-300'" />
                    </label>
                  </div>
                </div>
                <p class="field-hint">Clique na letra para marcar o gabarito · use $formula$ para LaTeX</p>
              </div>

              <Transition name="slide-down">
                <div v-if="manualError" class="error-box">
                  <Icon name="lucide:alert-circle" class="w-4 h-4 flex-shrink-0" />{{ manualError }}
                </div>
              </Transition>

              <button class="btn-submit"
                :class="[!questaoValida||savingManual?'btn-submit--disabled':'', editandoId!==null?'btn-submit--edit':'']"
        ...
        <div class="right-col">
          <div class="list-header">
            ...
            <div class="list-actions">
              <span v-if="progresso<100 && (progressInfo?.quota??0)>0" class="faltam-tag">
                faltam {{ Math.max(0,(progressInfo?.quota??0)-questoes.length) }}
              </span>
          ...
          <TransitionGroup name="list-item" tag="div" class="q-list">
            <div v-for="(q, i) in questoes" :key="q.id"
              class="q-card group" :class="editandoId===q.id?'q-card--editing':''">
...
// ── Imagens ────────────────────────────────────────────────────────────────
const uploadingImagem  = ref<string|null>(null)
const imagensEditando  = ref<any[]>([])
// Imagens adicionadas ANTES de salvar (ficam em memória até o POST da questão)
const imagensPendentes = ref<{file:File, context:string, previewUrl:string}[]>([])

const imagensDoEnunciado = computed(() =>
  // Se editando: usa imagens já salvas no servidor
  editandoId.value!==null
    ? imagensEditando.value.filter(i=>i.context==='stem')
    // Se novo: usa as pendentes locais
    : imagensPendentes.value.filter(i=>i.context==='stem').map(i=>({id:'p_'+i.previewUrl, url:i.previewUrl, context:'stem'}))
)

// ── LaTeX ─────────────────────────────────────────────────────────────────
const showLatexHelp = ref(false)
const stemRef       = ref<HTMLTextAreaElement|null>(null)
const latexExamples = [
  { code:'$x^2$',               label:'Potência'       },
  { code:'$\\sqrt{x}$',         label:'Raiz'           },
  { code:'$\\frac{a}{b}$',      label:'Fração'         },
  { code:'$\\pi$',              label:'Pi (π)'         },
  { code:'$\\alpha$',           label:'Alfa (α)'       },
  { code:'$\\theta$',           label:'Teta (θ)'       },
  { code:'$\\sum_{i=1}^{n}$',   label:'Somatório'      },
  { code:'$\\int_{a}^{b}$',     label:'Integral'       },
  { code:'$$E=mc^2$$',          label:'Bloco centrado' },
  { code:'$\\vec{v}$',          label:'Vetor'          },
  { code:'$\\infty$',           label:'Infinito'       },
  { code:'$\\neq$',             label:'Diferente (≠)'  },
  { code:'$\\leq$',             label:'Menor/igual (≤)'},
  { code:'$\\geq$',             label:'Maior/igual (≥)'},
  { code:'$\\Delta$',           label:'Delta (Δ)'      },
  { code:'$\\cdot$',            label:'Ponto (·)'      },
]

function inserirLatex(code: string) {
  const ta = stemRef.value
  if (!ta) { novaQuestao.stem += code; showLatexHelp.value = false; return }
  const s = ta.selectionStart ?? novaQuestao.stem.length
  const e = ta.selectionEnd   ?? novaQuestao.stem.length
  novaQuestao.stem = novaQuestao.stem.slice(0,s) + code + novaQuestao.stem.slice(e)
  nextTick(() => { ta.focus(); ta.setSelectionRange(s+code.length, s+code.length) })
  showLatexHelp.value = false
}

// ── Upload imagem ──────────────────────────────────────────────────────────
async function uploadImagem(event: Event, context: string) {
  const file = (event.target as HTMLInputElement).files?.[0]
  if (!file) return

  // Antes de salvar: guarda localmente e mostra preview
  if (editandoId.value === null) {
    const previewUrl = URL.createObjectURL(file)
    imagensPendentes.value.push({ file, context, previewUrl })
    showToast('Imagem adicionada — será enviada ao salvar.', 'info')
    ;(event.target as HTMLInputElement).value = ''
    return
  }

  uploadingImagem.value = context
  try {
    const fd = new FormData(); fd.append('file', file); fd.append('context', context)
    const img = await upload<any>(`/exams/${examId}/questions/${editandoId.value}/images`, fd)
    imagensEditando.value.push(img)
    const idx = questoes.value.findIndex(q => q.id === editandoId.value)
    if (idx !== -1) questoes.value[idx] = { ...questoes.value[idx], has_images:true, images:[...(questoes.value[idx].images??[]), img] }
    showToast('Imagem adicionada!')
  } catch (e:any) { showToast(e.message??'Erro ao enviar imagem.','error') }
  finally { uploadingImagem.value=null; (event.target as HTMLInputElement).value='' }
}

async function deletarImagem(imgId: number) {
  if (!editandoId.value) return
  try {
    await deleteReq(`/exams/${examId}/questions/${editandoId.value}/images/${imgId}`)
    imagensEditando.value = imagensEditando.value.filter(i=>i.id!==imgId)
    const idx = questoes.value.findIndex(q=>q.id===editandoId.value)
    if (idx !== -1) {
      const imgs = (questoes.value[idx].images??[]).filter((i:any)=>i.id!==imgId)
      questoes.value[idx] = { ...questoes.value[idx], images:imgs, has_images:imgs.length>0 }
    }
  } catch (e:any) { showToast(e.message??'Erro ao remover imagem.','error') }
}
...
const uploadedFileName = ref(''); const uploadError = ref(''); const isDragging = ref(false)
const savingManual     = ref(false); const manualError = ref('')
const editandoId       = ref<number|null>(null); const enviado = ref(false)
const confirmandoDelecaoId = ref<number|null>(null); const confirmandoLimpar = ref(false)
...
// FIX: progresso baseado no tamanho real da lista de questões
const progresso = computed(()=>{
  const quota=progressInfo.value?.quota??0
  if (!quota) return 0
  return Math.min(100, Math.round(questoes.value.length/quota*100))
})
  ...
  try {
    ...
    if (myAssignment) {
      ...
      if (progressData?.disciplines?.length) {
        ...
        // FIX: submitted = tamanho real da lista
        progressInfo.value={
          ...myDisc,
      ...
      const quotaCompleta=progressInfo.value&&progressInfo.value.quota>0&&questoes.value.length>=progressInfo.value.quota
...
async function salvarQuestao(){
  ...
  const payload={
    ...
    options:novaQuestao.options.slice(0,count).map(o=>({label:o.label,text:o.text.trim()})),
  ...
  try{
    if(editandoId.value!==null){
      await patch<any>(`/exams/${examId}/questions/${editandoId.value}`,payload)
      editandoId.value=null;imagensEditando.value=[]
      showToast('Questão atualizada!')
    }else{
      const created=await post<any>(`/exams/${examId}/questions`,payload)

      // Envia imagens pendentes (adicionadas antes de salvar) para a questão recém-criada
      if(imagensPendentes.value.length>0){
        for(const item of imagensPendentes.value){
          const fd=new FormData();fd.append('file',item.file);fd.append('context',item.context)
          await upload<any>(`/exams/${examId}/questions/${created.id}/images`,fd).catch(()=>{})
        }
    ...
    resetForm()
    // Sempre recarrega a lista do servidor para garantir renderização correta
    questoes.value=(await get<any[]>(`/exams/${examId}/questions`)).map(normalizeQuestion)
    await recarregarProgresso()
  }catch(e:any){manualError.value=e.message??'Erro ao salvar questão.';showToast(manualError.value,'error')}
...
function editarQuestao(q: any){
  ...
  imagensEditando.value=[...(q.images??[])]
  manualError.value=''
  window.scrollTo({top:0,behavior:'smooth'})
}
function cancelarEdicao(){editandoId.value=null;imagensEditando.value=[];resetForm()}
...
async function removerQuestao(id: number){
  ...
  try{
    ...
    await recarregarProgresso();showToast('Questão removida.')
  }catch(e:any){showToast(e.message??'Erro ao remover.','error')}
}
async function excluirTodasQuestoes(){
  confirmandoLimpar.value=false;let removidas=0
...
function resetForm(){
  novaQuestao.stem='';novaQuestao.options=letras.map(l=>({label:l,text:''}));novaQuestao.correct_label='A'
  manualError.value='';stemPreview.value='';showLatexHelp.value=false
  // Revoga URLs de preview para liberar memória
  imagensPendentes.value.forEach(i=>URL.revokeObjectURL(i.previewUrl))
  imagensPendentes.value=[]
}
...
async function recarregarProgresso(){
  try{
    ...
    if(data?.disciplines?.length){
      ...
      // FIX: submitted = tamanho real da lista (não filtra por campo)
      progressInfo.value={
        ...myDisc,
        submitted:      questoes.value.length,
        class_id:       classId,
        ...
        discipline_name:myAssignment?.discipline_name??progressInfo.value?.discipline_name,
        targetClassIds: progressInfo.value?.targetClassIds??[classId],
      }
    }
  }catch{}
}
...
.btn-completion{display:inline-flex;align-items:center;gap:.4rem;padding:.5rem 1rem;background:white;color:#059669;font-size:.78rem;font-weight:700;border-radius:.75rem;border:none;cursor:pointer}
...
@media(min-width:1024px){.left-col{position:sticky;top:1.5rem;max-height:calc(100vh - 3rem);overflow-y:auto}}
...
.req{color:#f87171}
.field-textarea{padding:.625rem .875rem;border:1px solid #e5e7eb;border-radius:.75rem;font-size:.8rem;outline:none;resize:vertical;line-height:1.6;transition:border-color .15s;width:100%}
...
/* Toolbar */
.toolbar{display:flex;align-items:center;gap:.375rem;flex-shrink:0}
.toolbar-btn{display:inline-flex;align-items:center;gap:.3rem;padding:.25rem .625rem;font-size:.68rem;font-weight:600;color:#6b7280;background:#f3f4f6;border:none;border-radius:.5rem;cursor:pointer;transition:all .13s;white-space:nowrap}
.toolbar-btn:hover:not(.toolbar-btn--disabled){background:#e5e7eb;color:#374151}
.toolbar-btn--disabled{opacity:.4;cursor:not-allowed}
.math-sym{font-size:.9rem}
/* LaTeX help */
.latex-help{background:#f8fafc;border:1px solid #e2e8f0;border-radius:.875rem;padding:.875rem}
.latex-help-header{display:flex;align-items:center;justify-content:space-between;font-size:.72rem;font-weight:700;color:#374151;margin-bottom:.625rem}
.latex-help-header button{background:none;border:none;cursor:pointer;color:#9ca3af}
.latex-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:.35rem}
.latex-ex{display:flex;flex-direction:column;gap:.2rem;padding:.4rem .5rem;background:white;border:1px solid #e5e7eb;border-radius:.5rem;cursor:pointer;text-align:left;transition:all .13s}
.latex-ex:hover{border-color:#93c5fd;background:#eff6ff}
.latex-ex code{font-size:.68rem;color:#2563eb;font-family:monospace}
.latex-ex span{font-size:.62rem;color:#9ca3af}
/* Imagens */
.images-strip{display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.375rem}
.image-thumb{position:relative;width:4rem;height:4rem;border-radius:.5rem;overflow:hidden;border:1px solid #e5e7eb}
.image-thumb--pending{border-color:#fbbf24;border-style:dashed}
.image-thumb img{width:100%;height:100%;object-fit:cover}
.img-del{position:absolute;top:.15rem;right:.15rem;width:1.25rem;height:1.25rem;border-radius:9999px;background:rgba(0,0,0,.55);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:white}
.pending-badge{position:absolute;bottom:0;left:0;right:0;background:rgba(245,158,11,.85);color:white;font-size:.55rem;font-weight:700;text-align:center;padding:.1rem}
/* Alternativas */
...
.opt-img-btn{width:1.75rem;height:1.75rem;border-radius:.5rem;border:1px dashed #e5e7eb;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .13s;flex-shrink:0}
.opt-img-btn:hover:not(.opt-img-btn--disabled){border-color:#93c5fd;background:#eff6ff}
.opt-img-btn--disabled{opacity:.4;cursor:not-allowed}
...
.btn-danger-sm{display:inline-flex;align-items:center;gap:.25rem;padding:.3rem .625rem;font-size:.68rem;font-weight:700;color:#ef4444;background:#fef2f2;border:none;border-radius:.5rem;cursor:pointer}
...
.img-tag{font-size:.62rem;font-weight:700;padding:.1rem .4rem;border-radius:9999px;background:#ede9fe;color:#5b21b6;display:inline-flex;align-items:center;gap:.2rem}
.source-tag{font-size:.62rem;font-weight:700;padding:.1rem .4rem;border-radius:9999px;background:#f3f4f6;color:#6b7280}
...
.slide-down-enter-to,.slide-down-leave-from{max-height:24rem}
