...
backend/
  app/
    ...
    models/         # ORM (users/roles, school, exam, refresh_token)
    routes/         # endpoints FastAPI (auth, school, exam, pdf, students/import, ...)
    schemas/        # Pydantic
    services/       # pdf_generator etc.
    main.py         # FastAPI app
  alembic/          # migrations
  scripts/
    entrypoint.sh   # inicia API e roda alembic upgrade
    wait-for.sh     # aguarda Postgres
  storage/          # PDFs gerados (volume no Docker)
  Dockerfile
  docker-compose.yml
  requirements.txt
  .env.sample

Dica: mantenha a documentação técnica detalhada dentro de backend/README.md também (opcional).
O README da raiz é a “vitrine” do projeto no GitHub.
...
1. Crie seu .env a partir do .env.sample:
    ...
    Importante: para Docker, mantenha POSTGRES_HOST=db e DATABASE_URL apontando para db:5432 (rede do compose).
...
Crie backend/app/startup_seed.py:
            # app/startup_seed.py
...
No backend/app/main.py, chame no startup:
    ...
    Assim, quando o banco estiver vazio (ex.: após docker compose down -v), roles e admin padrão são recriados automaticamente.
...
Vínculo institucional (professor ↔ classe ↔ disciplina)
    CRUD próprio ou inserts na teacher_class_subject.
...
Crie backend/.github/workflows/ci.yml:
    ...
    Em próximos passos, podemos incluir testes e build da imagem Docker no CI.



Makefile (opcional)
Crie backend/Makefile:
...
Contato & Próximos Passos
    ✅ Docker e Git prontos
    ✅ Sprint 1 e 2 operacionais
    ✅ Import CSV e PDFs funcionando

Próximo passo: implementar o Sprint 3 (OMR).