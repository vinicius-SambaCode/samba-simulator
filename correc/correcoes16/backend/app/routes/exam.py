...
def exam_progress(exam_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # ── FIX: contar questões por (discipline_id, class_id) individualmente ──────
    # Uma questão existe por turma — para o progresso do coordenador, usamos a
    # turma de referência (primeira turma atribuída por disciplina).
    # Isso evita multiplicar a contagem pelo nº de turmas.
    #
    # Mapa: discipline_id -> class_id de referência (primeira turma atribuída)
    ref_class: Dict[int, int] = {}
    for a in exam.teacher_assignments:
        if a.discipline_id not in ref_class:
            ref_class[a.discipline_id] = a.class_id
    # Fallback: usa a primeira turma do exam para disciplinas sem assignment
    first_class_id = exam.class_assignments[0].class_id if exam.class_assignments else None

    q_counts = (
        db.query(Question.discipline_id, Question.class_id, func.count(Question.id))
          .filter(Question.exam_id == exam.id, Question.state.in_(("submitted", "approved")))
          .group_by(Question.discipline_id, Question.class_id)
          .all()
    )
    # Mapa: (discipline_id, class_id) -> count
    counts_by_disc_class: Dict[tuple, int] = {}
    for disc_id, cls_id, cnt in q_counts:
        counts_by_disc_class[(disc_id, cls_id)] = cnt

    def _submitted_for_disc(disc_id: int) -> int:
        cid = ref_class.get(disc_id) or first_class_id
        if cid is None:
            # Nenhuma turma — soma tudo (fallback seguro)
            return sum(v for (d, _), v in counts_by_disc_class.items() if d == disc_id)
        return counts_by_disc_class.get((disc_id, cid), 0)

    items = []
    for disc_id, quota in quotas.items():
        submitted = _submitted_for_disc(disc_id)
        items.append({
...
# ----------------------------
# Excluir simulado (só antes de travar)
# ----------------------------
@router.delete("/{exam_id}", dependencies=[Depends(require_role("COORDINATOR", "ADMIN"))])
def delete_exam(exam_id: int, db: Session = Depends(get_db)):
    """Exclui simulado. Só permitido antes do travamento."""
    exam = _get_exam_or_404(db, exam_id)
    if exam.status not in (ExamStatus.COLLECTING, ExamStatus.REVIEW):
        raise HTTPException(
            status_code=400,
            detail="Simulado não pode ser excluído após o travamento."
        )
    db.delete(exam)  # cascade="all, delete-orphan" nos relationships cuida do resto
    db.commit()
    return {"detail": "Simulado excluído com sucesso."}
