...
async function generateBooklets() {
  ...
  const classNames = examClasses.value.map((c: any) => c.class_name).join(', ')
  startBatchTimer(`Gerando cadernos — ${classNames}`)
  try {
    ...
    for (const classId of classIds) {
      const cls = examClasses.value.find((c: any) => c.class_id === classId)
      batchOverlayMsg.value = `Gerando cadernos — ${cls?.class_name ?? classId}`
