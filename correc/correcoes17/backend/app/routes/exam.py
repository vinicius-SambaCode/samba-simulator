...
def exam_progress(exam_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Turma de referência por disciplina: primeira turma atribuída ao exam
    # Uma questão existe POR TURMA — para o progresso usamos apenas uma turma
    # de referência por disciplina, evitando multiplicar a contagem.
    ...
    # Fallback: primeira turma do exam para disciplinas sem assignment
    first_class_id = exam.class_assignments[0].class_id if exam.class_assignments else None

    # Contagem por (discipline_id, class_id)
    q_counts = (
    ...
    counts_by_key: Dict[tuple, int] = {
        (d, c): n for d, c, n in q_counts
    }

    def _submitted(disc_id: int) -> int:
        cid = ref_class.get(disc_id) or first_class_id
        if cid is None:
            # nenhuma turma configurada: soma tudo como fallback
            return sum(v for (d, _), v in counts_by_key.items() if d == disc_id)
        return counts_by_key.get((disc_id, cid), 0)

    items = []
    for disc_id, quota in quotas.items():
        submitted = _submitted(disc_id)
        items.append({
...
# ----------------------------
# Excluir simulado (apenas antes do travamento)
# ----------------------------
...
def delete_exam(exam_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(exam)  # cascade="all, delete-orphan" nos relationships cuida de tudo
    db.commit()
