...
# ---------------------------------------------------------------------------
# POST /auth/change-password  — troca de senha (obrigatória ou voluntária)
# ---------------------------------------------------------------------------
...
def change_password(
    ...
    if len(pwd) < 8 or len(pwd) > 16:
        errors.append("entre 8 e 16 caracteres")
    ...
    if errors:
        raise HTTPException(status_code=422, detail=f"Senha fraca: {', '.join(errors)}.")
