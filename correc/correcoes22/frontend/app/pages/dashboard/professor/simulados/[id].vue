...
<template>
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <!-- ===================================================== -->
    <!-- MODAL: Professor com múltiplas turmas                  -->
    <!-- ===================================================== -->
    <Transition name="pop">
      <div v-if="showClassSelector"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
              <h3 class="font-bold text-gray-900">Para qual turma você quer enviar?</h3>
              <p class="text-xs text-gray-400 mt-0.5">Você está vinculado a múltiplas turmas neste simulado</p>
            </div>
            <button @click="showClassSelector = false"
          ...
          <div class="space-y-2">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Opções de envio</p>

            <!-- Opção: todas as turmas do mesmo ano -->
            <button v-if="turmasDoMesmoAno.length > 1"
              class="w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-blue-200 bg-blue-50 text-left hover:bg-blue-100 transition-colors"
              @click="selecionarTodasDoAno">
              <Icon name="lucide:layers" class="w-5 h-5 text-blue-600 flex-shrink-0" />
              <div>
                <p class="text-sm font-semibold text-blue-900">Mesmas questões para todas as turmas do mesmo ano</p>
                <p class="text-xs text-blue-600">{{ turmasDoMesmoAno.map(t => t.class_name).join(', ') }}</p>
              </div>
            </button>

            <!-- Opção: turma específica -->
            <button v-for="a in myAssignments" :key="a.class_id"
              class="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 text-left hover:border-blue-300 hover:bg-blue-50/50 transition-colors"
              @click="selecionarTurma(a)">
              <Icon name="lucide:door-open" class="w-5 h-5 text-gray-400 flex-shrink-0" />
              <div>
                <p class="text-sm font-semibold text-gray-800">Apenas {{ a.class_name }}</p>
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between px-1">
          ...
          <div class="flex items-center gap-2">
            <button v-if="questoes.length > 0 && !confirmandoLimpar"
              @click="confirmandoLimpar = true"
              class="text-xs text-red-600 bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded-full font-medium flex items-center gap-1 transition-colors">
              <Icon name="lucide:trash-2" class="w-3 h-3" />
              Excluir todas
            </button>
            <div v-else-if="confirmandoLimpar" class="flex items-center gap-1.5">
              <span class="text-xs text-red-700 font-medium">Confirmar?</span>
              <button @click="excluirTodasQuestoes"
                class="text-xs text-white bg-red-500 hover:bg-red-600 px-2.5 py-1 rounded-full font-medium transition-colors">
                Sim
              </button>
              <button @click="confirmandoLimpar = false"
                class="text-xs text-gray-600 bg-gray-100 hover:bg-gray-200 px-2.5 py-1 rounded-full font-medium transition-colors">
                Não
              </button>
            </div>
...
const turmasDoMesmoAno   = computed(() => {
  ...
  // Agrupa por ano (ex: "3A", "3B" → ano "3"; "9º A", "9º B" → ano "9")
...
function selecionarTurma(assignment: any) {
  progressInfo.value = {
    ...progressInfo.value,
    ...
    targetClassIds: [assignment.class_id],
  }
...
function selecionarTodasDoAno() {
  ...
  progressInfo.value = {
    ...
    class_name:     turmasDoMesmoAno.value.map(t => t.class_name).join(', '),
    discipline_name: first.discipline_name,
    targetClassIds: turmasDoMesmoAno.value.map(t => t.class_id),
  }
  showClassSelector.value = false
}
...
onMounted(async () => {
  ...
  try {
    ...
    if (myAssignment) {
      // Salva lista completa de turmas do professor
      ...
      if (progressData?.disciplines?.length) {
        ...
        const mySubmitted = questoes.value.filter(
          (q: any) => q.discipline_id === myAssignment.discipline_id && q.class_id === myAssignment.class_id
        ).length
      ...
      // Se professor tem múltiplas turmas E cota não completa, exibe modal de seleção
      const quotaCompleta = progressInfo.value && progressInfo.value.quota > 0 && progressInfo.value.submitted >= progressInfo.value.quota
      if (myAssignments.value.length > 1 && !quotaCompleta) {
...
async function excluirTodasQuestoes() {
  ...
  const ids = questoes.value.map((q: any) => q.id)
  let removidas = 0
  for (const id of ids) {
    try {
      await deleteReq(`/exams/${examId}/questions/${id}`)
      questoes.value = questoes.value.filter((q: any) => q.id !== id)
      removidas++
    } catch {}
  }
  await recarregarProgresso()
  showToast(`${removidas} questão(ões) removida(s).`)
}
...
async function recarregarProgresso() {
  try {
    ...
    if (data?.disciplines?.length) {
      ...
      const mySubmitted = questoes.value.filter(
        (q: any) => q.discipline_id === discId && q.class_id === (myAssignment?.class_id ?? progressInfo.value?.class_id)
      ).length
